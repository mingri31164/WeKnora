export default {
  ...
  knowledgeBase: {
    ...
    batchCancelParse: "파싱 중지",
    confirmBatchCancelParse: "파싱 중지",
    confirmBatchCancelParseDocument: "선택한 {count}개 항목의 파싱을 중지하시겠습니까? 이미 완료된 항목은 자동으로 건너뜁니다.",
    batchCancelParseSuccess: "{count}개 파싱을 중지했습니다",
    batchCancelParseSkipped: "{cancelled}개 중지, {skipped}개 건너뜀(진행 중 아님)",
    batchCancelParseNone: "선택 항목 중 파싱 중인 지식이 없습니다",
    batchCancelParseFailed: "일괄 파싱 중지 실패",
    buildProgressLabel: "구축 중 {completed}/{total}",
    buildProgressAllDone: "모두 완료 {total}/{total}",
    statusCompleted: "완료",
  ...
  integrations: {
    ...
    api: {
      ...
      apiKeyAccessTypeHint: "\"지식베이스\"는 아래 권한 수준에 따라 지식베이스 범위 내에서 작동합니다. \"테넌트 전체 접근\"은 모델, 데이터 소스 등 모든 테넌트 수준 API를 추가로 사용할 수 있습니다.",
      accessTypeKnowledgeBase: "지식베이스",
      accessTypeTenantFull: "테넌트 전체 접근",
      apiKeyPermissionLevel: "권한 수준",
      apiKeyAccessRoleHint: "앞의 세 역할은 지식베이스 범위 내 작업으로 제한되며, \"테넌트 전체 접근\"은 모델, 데이터 소스 등 테넌트 수준 API도 추가로 사용할 수 있습니다.",
      accessRoleViewer: "지식베이스 읽기",
      accessRoleContributor: "지식베이스 읽기·쓰기",
      accessRoleAdmin: "지식베이스 관리자",
      accessRoleOwner: "테넌트 전체 접근",
      accessRoleViewerDesc: "조회, 검색 및 대화만 가능하며 지식베이스 내용은 수정하지 않습니다.",
      accessRoleContributorDesc: "선택한 범위 내에서 지식베이스 콘텐츠를 생성, 수정 또는 삭제합니다.",
      accessRoleAdminDesc: "선택한 범위 내에서 지식베이스를 관리하며, 지식베이스 비우기 등 파괴적 작업을 포함합니다. 테넌트 수준 설정은 다루지 않습니다.",
	      accessRoleOwnerDesc: "전체 접근, 테넌트 소유자와 동일: 모델, 벡터 스토어, 데이터 소스, 채널 등 모든 테넌트 수준 API를 호출할 수 있으며 지식베이스 범위의 제한을 받지 않습니다.",
	      ...
	      apiKeyCapabilityGroupTenant: "테넌트 설정",
	      apiKeyCapabilitySelectGroup: "전체 선택",
	      apiKeyCapabilityClearGroup: "비우기",
	      capabilityTenantFull: "테넌트 전체 접근",
	      capabilityTenantFullHint: "모델, 벡터 스토어, 데이터 소스, 채널 등 모든 테넌트 수준 API를 허용합니다. 지식베이스 범위는 적용되지 않습니다.",
      ...
      capabilityManageKbsHint: "선택한 범위 안에서 기존 지식베이스를 수정 또는 삭제하고 초기화/설정을 변경할 수 있습니다. 새 지식베이스 생성은 허용하지 않습니다.",
	      ...
	      capabilityMessageHistoryHint: "테넌트 채팅 기록 검색과 채팅 기록 통계 조회를 허용합니다. 테넌트 설정 권한은 부여하지 않습니다.",
	      ...
	      capabilityManageMembersHint: "테넌트 멤버, 역할, 초대 및 초대 링크를 조회하고 관리할 수 있습니다. API Key 관리, 테넌트 삭제 또는 소유권 이전은 포함하지 않습니다.",
	      ...
	      capabilityManageTenantSettings: "테넌트 설정 관리",
	      capabilityManageTenantSettingsHint: "API 최종 사용자 식별 모드, 요청 헤더 설정, 테넌트 KV 설정 등 테넌트 수준 통합 설정을 읽고 업데이트합니다. API Key 관리, 멤버 관리, 테넌트 삭제 또는 소유권 이전은 포함하지 않습니다.",
