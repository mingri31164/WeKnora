export default {
  workbench: {
    artifactKinds: { presentation: 'Презентация', web_page: 'Веб-страница', spreadsheet: 'Таблица', document: 'Документ', image: 'Изображение', text: 'Текст', other: 'Файл' },
    title: 'Рабочая панель', terminal: 'Терминал', files: 'Файлы', artifacts: 'Артефакты', audit: 'Аудит',
    open: 'Открыть рабочую панель',
    reinitialize: 'Повторить инициализацию песочницы',
    reinitializeHint: 'Повторить инициализацию без изменения конфигурации, привязанной к этой сессии.',
    refresh: 'Обновить', connect: 'Подключить', disconnect: 'Отключить', clearOutput: 'Очистить вывод',
    terminalOutput: 'Вывод терминала', command: 'Команда', run: 'Запустить', interrupt: 'Прервать',
    outputLimit: 'Достигнут лимит вывода.', noReplay: 'Соединение закрыто. Команды не запускаются повторно.',
    exitCode: 'Код выхода {code}', back: 'Назад', parentDirectory: 'Родительский каталог', upload: 'Загрузить файл',
    newDirectory: 'Создать каталог', emptyDirectory: 'Каталог пуст', fileActions: 'Действия с файлом',
    destination: 'Путь относительно /workspace/output', rename: 'Переименовать / переместить', delete: 'Удалить', download: 'Скачать',
    deleteConfirm: 'Удалить {path}? Каталог должен быть пустым.', invalidPath: 'Укажите относительный путь без .., пустых сегментов и обратных косых черт.',
    noOverwrite: 'Путь уже существует. Перезапись запрещена.', singleFileOnly: 'Перетащите один файл. Загрузка папок не поддерживается.',
    fileTooLarge: 'Файл превышает {size}.', requestFailed: 'Ошибка запроса. Повторите попытку или проверьте права.',
    officeExternal: 'Предпросмотр заблокирован: файл Office содержит внешние ссылки или ресурсы. Используйте файл со встроенными ресурсами.',
    officeInvalid: 'Предпросмотр заблокирован: файл Office поврежден или содержит небезопасные пути в архиве.',
    officeTooLarge: 'Предпросмотр заблокирован: файл Office превышает ограничения размера архива или распакованных данных.',
    imageTooLarge: 'Предпросмотр заблокирован: изображение превышает ограничение размера или числа пикселей.',
    noArtifacts: 'В этой сессии нет артефактов', missingArtifactIdentity: 'Нет идентификатора предпросмотра', noAudit: 'Нет записей аудита',
    selectConfig: 'Выберите конфигурацию песочницы', bind: 'Привязать к сессии', noConfigs: 'Нет доступных конфигураций песочницы',
    limits: 'Ограничения', commandTimeout: 'Время команды', sessionTimeout: 'Время сессии', cpuLimit: 'Время CPU', memoryLimit: 'Память (RSS дерева / AS процесса)', fileLimit: 'Размер файла',
    memoryLimitHint: 'Один лимит ограничивает адресное пространство каждого процесса и периодически измеряемую сумму RSS команды и её потомков. При достижении лимита RSS всё дерево команды завершается. Общие страницы учитываются повторно, между замерами возможен перерасход. Это не жёсткая квота памяти контейнера или всей песочницы.',
    terminalUnavailable: 'Терминал недоступен для этой песочницы', filesUnavailable: 'Файлы недоступны для этой песочницы',
    states: {
      bound: 'Привязано', ready: 'Готово', available: 'Доступно', disabled: 'Рабочая панель отключена',
      unbound: 'Песочница не привязана', policy_denied: 'Заблокировано политикой рабочего пространства',
      unavailable: 'Рабочая панель недоступна', unsupported: 'Песочница не поддерживается',
      capability_unavailable: 'Возможности песочницы недоступны. Проверьте конфигурацию и повторите инициализацию.',
    },
    connection: { disconnected: 'Отключено', connecting: 'Подключение', ready: 'Ожидание', starting: 'Запуск', running: 'Выполняется' },
  },
  localBrowser: {
